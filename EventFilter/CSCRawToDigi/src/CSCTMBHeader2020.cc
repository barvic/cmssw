...
const std::vector<std::pair<unsigned,unsigned> > run3_pattern_lookup_tbl = 
   {{0,0}, {0,1}, {0,2}, {0,3}, {0,4}, /* Valid LCT0, invalid LCT1 combination. Check LCT1 vpf */
    {0,0}, {0,1}, {0,2}, {0,3}, {0,4},  
    {1,0}, {1,1}, {1,2}, {1,3}, {1,4},
    {2,0}, {2,1}, {2,2}, {2,3}, {2,4},
    {3,0}, {3,1}, {3,2}, {3,3}, {3,4},
    {4,0}, {4,1}, {4,2}, {4,3}, {4,4}}; /* pattern IDs 30,31 are reserved */

const unsigned run2_pattern_lookup_tbl[2][16] = 
   {{10,10,8,8,8,8,6,6,6,6,4,4,4,2,2,2},
...
CSCTMBHeader2020::CSCTMBHeader2020(const unsigned short * buf)
{
...
std::vector<CSCCLCTDigi> CSCTMBHeader2020::CLCTDigis(uint32_t idlayer) 
{
  ...
  CSCCLCTDigi digi0(bits.clct0_valid, bits.clct0_quality,
                    pattern, 1, bend, strip + (quart<<5) + (eight<<6), cfeb, bits.clct_bxn, 1, bits.bxnPreTrigger,
		    bits.clct0_comparator_code, CSCCLCTDigi::Version::Run3);
  ...
  CSCCLCTDigi digi1(bits.clct1_valid, bits.clct1_quality,
                    pattern, 1, bend, strip + (quart<<5) + (eight<<6), cfeb, bits.clct_bxn, 2, bits.bxnPreTrigger,
                    bits.clct1_comparator_code, CSCCLCTDigi::Version::Run3 );
...
CSCTMBHeader2020::CorrelatedLCTDigis(uint32_t idlayer) const 
{
    ...
    CSCCorrelatedLCTDigi digi(1, bits.MPC_Muon0_lct_vpf, bits.MPC_Muon0_lct_quality,
      bits.MPC_Muon0_alct_key_wire, strip, pattern,
      bits.MPC_Muon0_clct_LR, bits.MPC_Muon_alct_bxn, 0,
      bits.MPC_Muon0_clct_bx0, 0, 0, hmt, CSCCorrelatedLCTDigi::Version::Run3);
    ...
    //offlineHalfStripNumbering(strip);
    digi = CSCCorrelatedLCTDigi(2, bits.MPC_Muon1_lct_vpf, bits.MPC_Muon1_lct_quality,
      bits.MPC_Muon1_alct_key_wire, strip, pattern,
      bits.MPC_Muon1_clct_LR, bits.MPC_Muon_alct_bxn, 0,
      bits.MPC_Muon1_clct_bx0, 0, 0, hmt, CSCCorrelatedLCTDigi::Version::Run3);                     
...
void
CSCTMBHeader2020::addALCT0(const CSCALCTDigi & digi)
{
...
void
CSCTMBHeader2020::addALCT1(const CSCALCTDigi & digi)
{
  throw cms::Exception("In CSC TMBHeaderFormat 2007, ALCTs belong in  ALCT header");
}

void
CSCTMBHeader2020::addCLCT0(const CSCCLCTDigi & digi)
{
...
void
CSCTMBHeader2020::addCLCT1(const CSCCLCTDigi & digi)
{
  int halfStrip = digi.getKeyStrip();
  int pattern = digi.getPattern();
  //hardwareStripNumbering(strip, cfeb, pattern, bend);
...
void
CSCTMBHeader2020::addCorrelatedLCT0(const CSCCorrelatedLCTDigi & digi)
{
...
void
CSCTMBHeader2020::addCorrelatedLCT1(const CSCCorrelatedLCTDigi & digi)
{
  // int halfStrip = digi.getStrip();
  // hardwareHalfStripNumbering(halfStrip);
...
//
  os << "CLCT Words:\n"
     << " bits.clct0_valid = " << bits.clct0_valid
     << " bits.clct0_shape = " << bits.clct0_shape
     << " bits.clct0_quality = " << bits.clct0_quality 
     << " halfstrip = " << (bits.clct0_key_low + (bits.clct0_key_high << 7)) << "\n";
  os << " bits.clct0_xky = " << bits.clct0_xky
     << " bits.clct0_comparator_code = " << bits.clct0_comparator_code << "\n";

  os << " bits.clct1_valid = " << bits.clct1_valid
     << " bits.clct1_shape = " << bits.clct1_shape
     << " bits.clct1_quality = " << bits.clct1_quality 
     << " halfstrip = " << (bits.clct1_key_low + (bits.clct1_key_high << 7)) << "\n"; 
  os << " bits.clct1_xky = " << bits.clct1_xky 
     << " bits.clct1_comparator_code = " << bits.clct1_comparator_code << "\n";

  os << "MPC Words:\n"  
     << " LCT0 valid = " << bits.MPC_Muon0_lct_vpf 
     << " key WG = " << bits.MPC_Muon0_alct_key_wire
     << " key halfstrip = " << bits.MPC_Muon0_clct_key_halfstrip
     << " 1/4strip flag = " << bits.MPC_Muon0_clct_QuarterStrip << " 1/8strip flag = " << bits.MPC_Muon0_clct_EighthStrip << "\n"
  ...
  os << " LCT1 valid = " << bits.MPC_Muon1_lct_vpf
     << " key WG = " << bits.MPC_Muon1_alct_key_wire
     << " key halfstrip = " << bits.MPC_Muon1_clct_key_halfstrip
     << " 1/4strip flag = " << bits.MPC_Muon1_clct_QuarterStrip << " 1/8strip flag = " << bits.MPC_Muon1_clct_EighthStrip << "\n"
  ...
  // os << "..................CLCT....................." << "\n";
  os << "GEM Data:\n"
     << " gem_enabled_fibers = " <<  GEM_enabled_fibers() 
     << " gem_fifo_tbins = " << GEM_fifo_tbins() 
     << " gem_fifo_pretrig = " << GEM_fifo_pretrig() 
     << " gem_zero_suppress = " << GEM_zero_suppress() 
     << " gem_csc_bend_enable = " << bits.gem_csc_bend_enable
     << " gem_sync_dataword = 0x" << std::hex << GEM_sync_dataword() 
